...
function ensureModals() {
    if (document.getElementById('sm-story-modal')) return;
    ...
    const html = `
        <div class="mb-14">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div class="${themePanel} border ${themeBorder} rounded-2xl p-6 group hover:-translate-y-1 transition-all">
                    ...
                    <p class="text-[11px] opacity-50 leading-relaxed">Especialista en la humanización de servicios complejos. Transformé tecnicismos en contenido educativo y accesible. <em class="opacity-70">(Cootraim, Confecoop)</em></p>
                </div>
                <div class="${themePanel} border ${themeBorder} rounded-2xl p-6 group hover:-translate-y-1 transition-all">
                    <h4 class="font-serif italic text-base mb-2 font-bold">Moda y Gastronomía</h4>
                    <p class="text-[11px] opacity-50 leading-relaxed">Gestión de narrativa visual y sensorial. Storytelling del proceso creativo y experiencias de alto impacto. <em class="opacity-70">(Diseñamos, CBC)</em></p>
                </div>
                <div class="${themePanel} border ${themeBorder} rounded-2xl p-6 group hover:-translate-y-1 transition-all">
                    <h4 class="font-serif italic text-base mb-2 font-bold">Salud y Belleza</h4>
                    <p class="text-[11px] opacity-50 leading-relaxed">Construcción de autoridad y lealtad basados en la educación y la empatía. <em class="opacity-70">(Dra. Arias, Dra. Cepeda)</em></p>
                </div>
            </div>
        </div>
        ...
        <div class="mb-10">
            ...
            <div class="flex gap-5 overflow-x-auto pb-3" style="scrollbar-width:none;">
                ${brands.map(b => `
                    <div class="flex flex-col items-center gap-2 cursor-pointer group shrink-0" onclick="openSMStory('${b.id}')">
                        <div class="p-[2.5px] rounded-full bg-gradient-to-br ${b.gradient} hover:scale-105 transition-transform duration-300 shadow-lg">
                            <div class="w-[68px] h-[68px] rounded-full bg-[#0f0c29] p-[2px] overflow-hidden">
                                <img src="${epath(b.avatar)}" alt="${b.name}" class="w-full h-full rounded-full object-cover">
                            </div>
                        </div>
                        <span class="text-[9px] font-bold uppercase tracking-wide opacity-60 group-hover:opacity-100 text-center">${b.name}</span>
        ...
        <div class="flex items-center gap-2 mb-6 overflow-x-auto pb-1" style="scrollbar-width:none;">
            <button onclick="filterSMBrand('all')" id="sm-filter-all" class="shrink-0 px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest border transition-all bg-violet-600 text-white border-violet-600">Todos</button>
            ${brands.map(b => `
                <button onclick="filterSMBrand('${b.id}')" id="sm-filter-${b.id}" class="shrink-0 px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest border transition-all opacity-50 hover:opacity-100" style="border-color:rgba(99,86,255,0.2);">${b.name}</button>
            `).join('')}
        </div>

        <div id="sm-posts-grid" class="grid grid-cols-3 gap-1 mb-20">
            ${brands.flatMap(b => b.posts.map((post, i) => {
        ...
        return `
                    <div class="sm-post-item aspect-square overflow-hidden cursor-pointer relative group" data-brand="${b.id}" onclick="openSMPost(${clickArg})" style="background:${b.color}15;">
                        <img src="${src}" alt="${b.name} post ${i + 1}" class="w-full h-full object-cover transition-all duration-500 group-hover:scale-105 group-hover:brightness-75" loading="lazy">
                        ${isVideo ? `
        ...
        if (typeof srcOrObj === 'object' && srcOrObj.type === 'youtube') {
            contentContainer.innerHTML = `
                <div class="relative w-full max-w-4xl mx-auto aspect-[9/16] sm:aspect-video rounded-2xl overflow-hidden shadow-[0_25px_60px_rgba(0,0,0,0.8)] border border-white/10 bg-black">
                    <iframe src="https://www.youtube.com/embed/${srcOrObj.id}?autoplay=1&controls=0&rel=0&modestbranding=1&playsinline=1" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen class="absolute inset-0 w-full h-full"></iframe>
                </div>
            `;
        } else {
            contentContainer.innerHTML = `
